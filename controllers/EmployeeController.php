...
use app\models\WorkTableSearch;
use yii\helpers\Url;
...
class EmployeeController extends \yii\web\Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'allow' => true,
                        'actions' => ['worktable', 'punchtable', 'index', 'timecard', 'connect', 'worktable-post' ,'test', 'myworktable-ajax'],
                    ],
                    [
                        'allow' => true,
                        'actions' => ['myworktable'],
    ...
    public function actionMyworktableAjax()
    {
        ...
        if (isset($day) && $day != '') {
            $where[] = array('like', 'day', $day);
        }
        if (isset($item) && $item != '') {
            $where[] = array('like', 'work_item', $item);
        }
        if (isset($finish) && $finish != '') {
            $where[] = array('like', 'work_finish', $finish);
        }

        $dataProvider = WorkTable::find()->where($where)->asArray()->offset($offset)->limit(7)->orderBy('day DESC')->all();
        $intCount = WorkTable::find()->where($where)->count();
        ...
        //$pageList = ceil($intCount/7);
        //
        // ->offset(0)->limit(10)
        // find()->count();
        return json_encode($reJson);
        //return $intCount."▶@◀".$offset."▶@◀".json_encode($dataProvider);

    }
    public function actionMyworktable()
    {
        $request = Yii::$app->request;
        $id = Yii::$app->user->identity->id;
        //$dataProvider = WorkTable::findBySql('SELECT * FROM work_table WHERE employee_id ='.$id.' AND day = "'.$day.'"')->asArray()->all();

        $searchModel = new WorkTableSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        //$url = Url::to(['employee/myworktable']);
        $url = Url::to(['employee/myworktable-ajax']);
        ...
        return $this->render('myworktable', [
    ...
    public function actionWorktablePost()
    {
        ...
        if (Yii::$app->request->post()) {
            $query = WorkTable::findBySql('SELECT * FROM work_table WHERE employee_id ='.$id.' AND day = "'.$day.'"')->one();

            if (!$query) {
                $link = Yii::$app->db;

                // 忽略重複請求
                $sql = 'INSERT IGNORE INTO `work_table`(`employee_id`, `day`, `work_item`, `work_finish`) VALUES (:id, :day, :item, :finish)';
                $sth = $link->createCommand($sql);
                $sth->bindParam(':id', $id);
